...
function relaxGraph() {
  const EDGE_TARGET = 170;      // higher = linked blobs further apart
  const EDGE_FORCE = 0.0012;    // higher = edges pull harder
  const CENTER_PULL = 0.000006; // lower = graph spreads wider
  const REPULSE_RANGE = 95;     // higher = blobs avoid each other sooner
  const REPULSE_FORCE = 0.055;  // higher = stronger blob separation
  const COLLISION_PAD = 26;     // extra spacing around each orb/label
  const DAMPING = 0.86;         // lower = settles faster, higher = floatier
  const ITERS = 3;              // higher = cleaner spacing, more CPU

  for (let iter = 0; iter < ITERS; iter++) {
    // Pull connected nodes toward a readable edge length.
    for (const ed of graphEdges) {
      const dx = ed.b.x - ed.a.x;
      const dy = ed.b.y - ed.a.y;
      const d = Math.hypot(dx, dy) || 1;

      const force = (d - EDGE_TARGET) * EDGE_FORCE;
      const fx = dx / d * force;
      const fy = dy / d * force;
    ...
    // Push all blobs away from each other.
    for (let i = 0; i < graphNodes.length; i++) {
      ...
      for (let j = i + 1; j < graphNodes.length; j++) {
        ...
        const minDist = (a.radius || 10) + (b.radius || 10) + COLLISION_PAD;
        const repelDist = Math.max(REPULSE_RANGE, minDist);

        if (d < repelDist) {
          const overlap = repelDist - d;
          const force = overlap * REPULSE_FORCE;
          const fx = dx / d * force;
          const fy = dy / d * force;
    ...
    // Weak center pull so the graph does not drift forever.
